...
describe('HomeComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [HomeService, { provide: Router, useValue: routerSpy }],
  ...
  it('check ngOnInit', () => {
    component.ngOnInit();
    component.book$ = homeService.getProducts().pipe(
      pluck('data'),
      tap((data) => {
        component.booksData = data;
      })
    );
  });
  it('should check error message', () => {
    component.errorImage('', 0);
    expect(component.booksData[0].image).toEqual(
  ...
  it('should visit details', () => {
    component.viewDetails(component.booksData[0]._id);
    let id = component.booksData[0]._id;
    expect(routerSpy.navigate).toHaveBeenCalledWith([`/details/${id}`]);
  });
});
